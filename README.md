...
- FFmpeg（系统安装，用于封装格式解封装和帧信息提取）
  - H.266/VVC 编码需要 FFmpeg master + libvvenc；VVC 解码需要 FFmpeg >= n7.0（内置原生解码器）
  - 源码编译方法参见 [FFmpeg-Build-Guide.md](FFmpeg-Build-Guide.md)
...
macOS Homebrew 安装的 Python 默认启用了 PEP 668 外部管理限制，需要加 `--break-system-packages` 参数：

```bash
pip3 install --break-system-packages -e .
```

安装后即可全局使用 `stream-analysis` 命令。使用 editable 模式（`-e`），代码修改即时生效。
